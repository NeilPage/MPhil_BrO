...
dfBrO = np.column_stack((BrO_Surf_All,BrO_LtCol_All,Perc_All,Temp_All))
dfBrO = pd.DataFrame(dfBrO, columns = ['BrO_Surf_All','BrO_LtCol_All','Perc_All','Temp_All'], index = BrO_Surf_All.index)
dfBrO = dfBrO.dropna()
...
# Apply the filter
F1 = dfBrO['Temp_All'] < -20 
BrO_All_1 = dfBrO[F1]

F2 = (dfBrO['Temp_All'] >= -20) & (dfBrO['Temp_All'] < -15) 
BrO_All_2 = dfBrO[F2]

F3 = (dfBrO['Temp_All'] >= -15) & (dfBrO['Temp_All'] < -10) 
BrO_All_3 = dfBrO[F3]

F4 = (dfBrO['Temp_All'] >= -10) & (dfBrO['Temp_All'] < -5) 
BrO_All_4 = dfBrO[F4]

F5 = (dfBrO['Temp_All'] >= -5) & (dfBrO['Temp_All'] < 0) 
BrO_All_5 = dfBrO[F5]

F6 = (dfBrO['Temp_All'] >= 0) & (dfBrO['Temp_All'] < 5) 
BrO_All_6 = dfBrO[F6]

F7 = (dfBrO['Temp_All'] >= 5) & (dfBrO['Temp_All'] < 10) 
BrO_All_7 = dfBrO[F7]
...
Mean_All = pd.concat([Mean1,Mean2,Mean3,Mean4,Mean5,Mean6,Mean7], axis =1)
Mean_All = Mean_All.T
...
StDev_All = pd.concat([StDev1,StDev2,StDev3,StDev4,StDev5,StDev6,StDev7], axis =1)
...
plt.scatter(Temp_All, Perc_All, edgecolors='none', marker='o', c='b', s = 3.5)

# Plot the mean
ax.plot(Mean_All['Temp_All'], Mean_All['Perc_All'],  marker='o', linewidth=3, markersize = 0.5, c='r', linestyle='-', label='Mean')  # Surf BrO
...
ax.plot(Mean_All['Temp_All'], UL, 'r-', linewidth=1.5, alpha=0.3, label='_nolegend_')   # plot the upper limit
ax.plot(Mean_All['Temp_All'], LL, 'r-', linewidth=1.5, alpha=0.3, label='_nolegend_')   # plot the lower limit
ax.fill_between(Mean_All['Temp_All'], UL, LL, facecolor='r', alpha=0.3) # fill the distribution
...
plt.title('Relationshp between BrO and Temperature', fontsize=15, y=1.03, x=1.12)
...
# ScatterPlot the values
plt.scatter(Temp_All, BrO_LtCol_All, edgecolors='none', marker='o', c='b', s = 3.5)

# Plot the mean
ax.plot(Mean_All['Temp_All'], Mean_All['BrO_LtCol_All'],  marker='o', linewidth=3, markersize = 0.5, c='r', linestyle='-', label='Mean')  # Surf BrO
