...
# NO2 VMR
NO2 = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/SIPEXII/all_NO2/SIPEXII_NO2_VMR.csv',index_col=0) # NO2 data for SIPEXII (2012)

# NO2 Error
NO2_Error = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/SIPEXII/NO2_error/SIPEXII_NO2_error.csv', index_col=0) # NO2 error data for SIPEXII (2012)
...
# BrO AOD
BrO_AOD = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/SIPEXII/all_AOD/SIPEXII_AOD_338.csv',index_col=0) # AOD data for SIPEXII (2012)

# NO2_AOD
NO2_AOD = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/SIPEXII/all_AOD/SIPEXII_AOD_375.csv',index_col=0) # AOD data for SIPEXII (2012)

#---------
# SZA
#---------

DF4_SZA = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/SIPEXII/all_SZA/SIPEXII_SZA.csv',index_col=0) # SZA data for SIPEXII (2012)
...
# Filtered
y = SIPEXIIT.index # set the values for the y-axis
x = np.array(SIPEXIIT.dtypes.index) # set the values for the x-axis
z = SIPEXIIT.copy() # identify the matrix containing the z-values (BrO in ppMv)
...
plt.subplots_adjust(top=0.96,bottom=0.15,left=0.08,right=1.0,hspace=0.05, wspace=0.15)
gs = gridspec.GridSpec(ncols=2, nrows=1, width_ratios=[n, m])

#------------------------------
# Graph 1 (MEDIAN BrO - Period 1)
ax  = fig.add_subplot(gs[0]) # options graph 2 (vertical no, horizontal no, graph no)
ax2 = ax.twinx()
ax3 = ax.twinx()
...
# TIMEFRAME FOR THE X-AXIS
#plt.xlim(datetime(2012,9,23,0,1,0),datetime(2012,11,12,0,0,5)) # All Periods
plt.xlim(datetime(2012,9,23,0,1,0),datetime(2012,10,2,0,0,5)) # Period 1
#plt.xlim(datetime(2012,10,9,0,1,0),datetime(2012,10,20,0,0,5)) # Period 2
#plt.xlim(datetime(2012,11,10,0,1,0),datetime(2012,11,12,0,0,5)) # Period 3
...
ax.xaxis.set_label_coords(1.05, -0.125)
...
#------------------------------
# Graph 2 (MEDIAN BrO - Period 2)
ax  = fig.add_subplot(gs[1]) # options graph 2 (vertical no, horizontal no, graph no)
...
# Format ColorBar
clb1 = fig.colorbar(col1, extend='max',pad = 0.19)
...
# #------------------------------
# # Graph 1 (MEDIAN NO2 - Period 1)
# ax  = fig.add_subplot(gs[2]) # options graph 2 (vertical no, horizontal no, graph no)
# ax2 = ax.twinx()
# ax3 = ax.twinx()
# ax3.set_zorder(ax.get_zorder()+1)
# ax3.patch.set_visible(False)
...
# ax.xaxis.set_major_locator(mdates.DayLocator(interval=2)) # set the interval between dispalyed dates
# ax.xaxis.set_minor_locator(mdates.HourLocator(interval=4))
# ax.xaxis.set_label_coords(1.05, -0.225)
...
# # #------------------------------
# # # Graph 2 (MEDIAN NO2 - Period 2)
# # ax  = fig.add_subplot(gs[3]) # options graph 2 (vertical no, horizontal no, graph no)
# # #ax = plt.subplot(gs[1]) # options graph 2 (vertical no, horizontal no, graph no)
# ax2 = ax.twinx()
# ax3 = ax.twinx()
# ax3.set_zorder(ax.get_zorder()+1)
