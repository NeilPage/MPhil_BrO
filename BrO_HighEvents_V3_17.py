...
HYSPLIT_E12 = mpimg.imread('/Users/ncp532/Desktop/Graphs/HYSPLIT/Event12_20180201.gif')
HYSPLIT_E13 = mpimg.imread('/Users/ncp532/Desktop/Graphs/HYSPLIT/Event13_20180207.gif')
HYSPLIT_E14 = mpimg.imread('/Users/ncp532/Desktop/Graphs/HYSPLIT/Event14_20180211.gif')
...
for i in range(len(Date1)):
    date.append(datetime.strptime(Date1[i],'%d/%m/%Y %H:%M:%S')+ timedelta(hours=5))
...
for i in range(len(Date2)):
    date2.append(datetime.strptime(Date2[i],'%d/%m/%Y %H:%M:%S')+ timedelta(hours=5))
...
for i in range(len(Date3)):
    date3.append(datetime.strptime(Date3[i],'%d/%m/%Y %H:%M:%S')+ timedelta(hours=5))
...
for i in range(len(Date5)):
    date5.append(datetime.strptime(Date5[i],'%d/%m/%Y %H:%M:%S')+ timedelta(hours=5))
...
for i in range(len(Date4)):
    date4.append(datetime.strptime(Date4[i],'%d/%m/%Y %H:%M:%S')+ timedelta(hours=5))
...
for i in range(len(Date8)):
    date8.append(datetime.strptime(Date8[i],'%d/%m/%Y %H:%M:%S')+ timedelta(hours=5))
...
for i in range(len(Date10)):
    date10.append(datetime.strptime(Date10[i],'%d/%m/%Y %H:%M:%S')+ timedelta(hours=5))
...
end_date   = '2018-02-02'
Event12    = (SIPEXIITT.index >= start_date) & (SIPEXIITT.index < end_date)
Event12M   = SIPEXIITT[Event12]

# Event 13 (7 Feb 2018)
start_date = '2018-02-07'
end_date   = '2018-02-08'
Event13    = (SIPEXIITT.index >= start_date) & (SIPEXIITT.index < end_date)
Event13M   = SIPEXIITT[Event13]

# Event 14 (11 Feb 2018)
start_date = '2018-02-11'
end_date   = '2018-02-12'
Event14    = (SIPEXIITT.index >= start_date) & (SIPEXIITT.index < end_date)
Event14M   = SIPEXIITT[Event14]

# Transpose the dataframes back again
Event12TT = Event12M.T
Event13TT = Event13M.T
Event14TT = Event14M.T

#------------------------------------------------------------------------------
# Calculate the mean/StDev and median/MAD BrO VCD for each altitude 

# Event 12
Mean_Event12   = np.mean(Event12TT, axis=1) * 1e6
Median_Event12 = np.median(Event12TT, axis=1) * 1e6
StDev_Event12  = np.std(Event12TT, axis=1) * 1e6
MAD_Event12    = Event12TT.mad(axis=1, skipna='True') * 1e6
MAX_Event12    = np.max(Event12TT, axis=1) * 1e6

# Event 13
Mean_Event13   = np.mean(Event13TT, axis=1) * 1e6
Median_Event13 = np.median(Event13TT, axis=1) * 1e6
StDev_Event13  = np.std(Event13TT, axis=1) * 1e6
MAD_Event13    = Event13TT.mad(axis=1, skipna='True') * 1e6
MAX_Event13    = np.max(Event13TT, axis=1) * 1e6

# Event 14
Mean_Event14   = np.mean(Event14TT, axis=1) * 1e6
Median_Event14 = np.median(Event14TT, axis=1) * 1e6
StDev_Event14  = np.std(Event14TT, axis=1) * 1e6
MAD_Event14    = Event14TT.mad(axis=1, skipna='True') * 1e6
MAX_Event14    = np.max(Event14TT, axis=1) * 1e6

#------------------------------------------------------------------------------
# PLOT EVENT 12 (1 Febuary 2018)
#------------------------------------------------------------------------------

fig = plt.figure(figsize=(10,6))
fig.suptitle('1 Febuary 2018', fontsize=20, y=0.95)
...
# Format x-axis
plt.xlim(datetime(2018,2,1,0,0,0),datetime(2018,2,1,23,59,59))
...
ax.errorbar(MAX_Event12, MAX_Event12.index,   marker='o', c='black',  markersize = 3.0, ls='-', label ='Max BrO: 23.2 $\pm$ 3.5 pptv',   xerr=StDev_Event12,   capsize=2)
...
#------------------------------------------------------------------------------
# PLOT EVENT 13 (7 Febuary 2018)
#------------------------------------------------------------------------------

fig = plt.figure(figsize=(10,6))
fig.suptitle('7 Febuary 2018', fontsize=20, y=0.95)
gs = fig.add_gridspec(4, 3)
plt.subplots_adjust(wspace=0.05)
...
# Format x-axis
plt.xlim(datetime(2018,2,7,0,0,0),datetime(2018,2,7,23,59,59))
xmajor_formatter = mdates.DateFormatter('%H:%M') # format how the date is displayed
ax.xaxis.set_major_formatter(xmajor_formatter)
ax.xaxis.set_major_locator(mdates.HourLocator(interval=3)) # set the interval between dispalyed dates
ax.xaxis.set_minor_locator(mdates.HourLocator(interval=1))
...
# Plot BrO MAX vertical profile
ax.errorbar(MAX_Event13, MAX_Event13.index,   marker='o', c='black',  markersize = 3.0, ls='-', label ='Max BrO: 7.1 $\pm$ 1.6 pptv',   xerr=StDev_Event13,   capsize=2)
...
#------------------------------------------------------------------------------
# PLOT EVENT 14 (11 Febuary 2018)
#------------------------------------------------------------------------------

fig = plt.figure(figsize=(10,6))
fig.suptitle('11 Febuary 2018', fontsize=20, y=0.95)
gs = fig.add_gridspec(4, 3)
plt.subplots_adjust(wspace=0.05)
...
# Format x-axis
plt.xlim(datetime(2018,2,11,0,0,0),datetime(2018,2,11,23,59,59))
xmajor_formatter = mdates.DateFormatter('%H:%M') # format how the date is displayed
ax.xaxis.set_major_formatter(xmajor_formatter)
ax.xaxis.set_major_locator(mdates.HourLocator(interval=3)) # set the interval between dispalyed dates
ax.xaxis.set_minor_locator(mdates.HourLocator(interval=1))
...
# Plot BrO MAX vertical profile
ax.errorbar(MAX_Event14, MAX_Event14.index,   marker='o', c='black',  markersize = 3.0, ls='-', label ='Max BrO: 26.7 $\pm$ 4.0 pptv',   xerr=StDev_Event14,   capsize=2)
...
ax2.yaxis.set_major_locator(ticker.MultipleLocator(10))
ax2.yaxis.set_minor_locator(ticker.MultipleLocator(5))
ax2.set_ylim(960,990) # On Station
ax2.yaxis.label.set_color('orange')
ax2.tick_params(axis='y', which='both', colors='orange', labelsize=10)
ax3.spines["right"].set_color('orange')
