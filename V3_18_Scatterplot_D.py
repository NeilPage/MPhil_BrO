...
# V3_17 (2017-18)
V3_17_BrO = pd.read_csv('/Users/ncp532/Documents/Data/V1_17_Apriori/V3_18_Data.csv',header=0,encoding = 'unicode_escape')
V3_17_Met = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/V03/CAMMPCAN_V3_underway_60.csv')
V3_17_SI  = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/V3_18_M_SeaIce.csv')
V3_17_Hg  = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/CAMMPCAN_V3_Hg0_QAQC_18-19.csv')
V3_17_O3  = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/O3/V3_O3_1min.csv')
...
Davis1 = (V3_17T.index >= start_date1) & (V3_17T.index < end_date1)
Davis2 = (V3_17T.index >= start_date2) & (V3_17T.index < end_date2)
V3_17_Davis1 = V3_17T[Davis1]
V3_17_Davis2 = V3_17T[Davis2]
V3_17T = pd.concat([V3_17_Davis1,V3_17_Davis2], axis =0)

#------------------------------------------------------------------------------
#  Sample the dataframe for only times corresponding to the BrO values

Data1 = pd.merge(left=V3_17T,right=V3_17_Met, how='left', left_index=True, right_index=True)
Data2 = pd.merge(left=Data1,right=V3_17_SI, how='left', left_index=True, right_index=True)
Data3 = pd.merge(left=Data2,right=V3_17_Hg, how='left', left_index=True, right_index=True)
Data4 = pd.merge(left=Data3,right=V3_17_O3, how='left', left_index=True, right_index=True)
...
plt.annotate("O$_3$ and BrO:\n (slope: "+str("%7.4f"%(slopeD1))+" $\pm$"+str("%7.4f"%(std_errD1))+" pptv, r: "+str("%7.4f"%(rD1))+")", xy=(14.4,3.8), color='black', fontweight='bold')
...
plt.annotate("Temperature and BrO:\n (slope: "+str("%7.4f"%(slopeD2))+" $\pm$"+str("%7.4f"%(std_errD2))+" $^\circ$C, r: "+str("%7.4f"%(rD2))+")", xy=(-1.92,3.8), color='black', fontweight='bold')
...
plt.annotate("Wind vector direction and BrO:\n (slope: "+str("%7.4f"%(slopeD3))+" $\pm$"+str("%7.4f"%(std_errD3))+" $^\circ$, r: "+str("%7.4f"%(rD3))+")", xy=(16.5,4.0), color='black', fontweight='bold')
...
plt.annotate("Wind speed and BrO:\n (slope: "+str("%7.4f"%(slopeD4))+" $\pm$"+str("%7.4f"%(std_errD4))+" m/s, r: "+str("%7.4f"%(rD4))+")", xy=(4.8,4.0), color='black', fontweight='bold')
...
plt.annotate("Solar radiation and BrO:\n (slope: "+str("%7.4f"%(slopeD5))+" $\pm$"+str("%7.4f"%(std_errD5))+" W/m$^2$, r: "+str("%7.4f"%(rD5))+")", xy=(47.0,4.0), color='black', fontweight='bold')
...
plt.annotate("Sea Ice Concentration and BrO:\n (slope: "+str("%7.4f"%(slopeD7))+" $\pm$"+str("%7.4f"%(std_errD7))+" %, r: "+str("%7.4f"%(rD7))+")", xy=(5.0,4.0), color='black', fontweight='bold')
