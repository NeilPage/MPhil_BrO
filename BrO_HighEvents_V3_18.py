...
#---------
# BrO VMR
BrO_V1_17 = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/V3_18/all_BrO/V3_18_BrO_VMR.csv', index_col=0) # BrO data for CAMPCANN V1 (2018/19)

# BrO Error
Err_V1_17 = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/V3_18/BrO_error/V3_18_BrO_error.csv', index_col=0) # BrO error data for CAMPCANN V1 (2018/19)
...
#---------
AEC_V1_17 = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/V3_18/all_Aerosol/V3_18_AeroExt_338.csv',index_col=0) # Aerosol extinction data at 338 nm for CAMPCANN V1 (2017/18)

#---------
# AOD
#---------
AOD_V1_17 = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/V3_18/all_AOD/V3_18_AOD_338.csv',index_col=0) # AOD data for CAMMPCAN V1 (2018/19)

#---------
# SZA
#---------
SZA_V1_17 = pd.read_csv('/Users/ncp532/Documents/data/V1_17_APriori/V3_18/all_SZA/V3_18_SZA.csv',index_col=0) # SZA data for CAMPCANN V1 (2018/19)

#---------
# MET
#---------
Met_V1_17  = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/ShipTrack/V3_18_underway_60.csv', index_col=0)

#---------
# O3
#---------
O3_V1_17  = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/O3/V3_O3_1min.csv', index_col=0)

#---------
# Hg0
#---------
Hg0_V1_17 = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/CAMMPCAN_V3_Hg0_QAQC_18-19.csv', index_col=0)
...
# XARRAY
cubes1 = xr.open_dataset('/Users/ncp532/Documents/Data/Sea_Ice_Cover/Hamburg_ICDC/20190130_median5day.nc', decode_cf=False, engine='netcdf4')
cubes2 = xr.open_dataset('/Users/ncp532/Documents/Data/Sea_Ice_Cover/NSIDC/2018-19/seaice_conc_daily_icdr_sh_f18_20190130_v01r00.nc', decode_cf=False, engine='netcdf4')
...
# BrO
#---------
BrO_V1_17.index = (pd.to_datetime(BrO_V1_17.index, dayfirst=True) + timedelta(hours=5)) # Davis timezone is UT+7

#---------
# Aerosol extinction at 338 nm (BrO)
#---------
AEC_V1_17.index = (pd.to_datetime(AEC_V1_17.index, dayfirst=True) + timedelta(hours=5)) # Davis timezone is UT+7

#---------
# AOD
#---------
AOD_V1_17.index = (pd.to_datetime(AOD_V1_17.index, dayfirst=True) + timedelta(hours=5)) # Davis timezone is UT+7

#---------
# SZA
#---------
SZA_V1_17.index = (pd.to_datetime(SZA_V1_17.index, dayfirst=True) + timedelta(hours=5)) # Davis timezone is UT+7

#---------
# MET
#---------
Met_V1_17.index = (pd.to_datetime(Met_V1_17.index, dayfirst=True) + timedelta(hours=5)) # Davis timezone is UT+7

#---------
# O3
#---------
O3_V1_17.index  = (pd.to_datetime(O3_V1_17.index,  dayfirst=True) + timedelta(hours=5)) # Davis timezone is UT+7
...
# Back Trajectories
#-------------
Traj_E21['year']     = Traj_E21['Traj Year']
Traj_E21['month']    = Traj_E21['Traj Mon']
Traj_E21['day']      = Traj_E21['Traj Day']
Traj_E21['hour']     = Traj_E21['Traj Hour']
Traj_E21['minute']   = Traj_E21['Traj Min']
Traj_E21['DateTime'] = pd.to_datetime(Traj_E21[['year', 'month', 'day', 'hour', 'minute']])
Traj_E21.index       = (pd.to_datetime(Traj_E21['DateTime']) + timedelta(hours=5)) # Davis timezone is UT+7

#--------------
# SEA ICE CONTACT TIME
#--------------
SI_100m_V1_18.index   = (pd.to_datetime(SI_100m_V1_18.index,   dayfirst=True) + timedelta(hours=5)) # Davis timezone is UT+7
...
# Event 21 (30 Jan 2019)
start_date = '2019-01-30'
end_date   = '2019-01-31'
Event21   = (SIPEXIITT.index >= start_date) & (SIPEXIITT.index < end_date)
Event21M  = SIPEXIITT[Event21]

# Transpose the dataframes back again
Event21TT = Event21M.T
...
#HAMBURG
seaice_data = cubes1.variables['sea_ice_area_fraction'][0,:,:] # Sea Ice concentration (time,y,x)(1,664,632)
lats        = cubes1.latitude
lons        = cubes1.longitude

# # NSIDC
# seaice_data = cubes2.variables['seaice_conc_cdr'][0,:,:] # Sea Ice concentration (time,y,x)(1,664,632)
...
# Event 21
Mean_Event21   = np.mean(Event21TT, axis=1) * 1e6
Median_Event21 = np.nanmedian(Event21TT, axis=1) * 1e6
StDev_Event21  = np.std(Event21TT, axis=1) * 1e6
MAD_Event21    = Event21TT.mad(axis=1, skipna='True') * 1e6
MAX_Event21    = np.max(Event21TT, axis=1) * 1e6
MIN_Event21    = np.min(Event21TT, axis=1) * 1e6

#------------------------------------------------------------------------------
# PLOT EVENT 21 (30 January 2019)
#------------------------------------------------------------------------------
fig = plt.figure(figsize=(10,6))
fig.suptitle('30 January 2019', fontsize=20, y=0.95)
...
start  = ax.axvline(datetime(2019,1,30,8,50,0), linewidth=1.0, color='r')
finish = ax.axvline(datetime(2019,1,30,11,10,0), linewidth=1.0, color='r')

# Text box for Start/Finish
props = dict(boxstyle='round', facecolor='white', alpha=1.0)
ax3.text(datetime(2019,1,30,8,50,0), 113, "Start",  color='r', fontsize=10, verticalalignment='top', horizontalalignment='center', bbox=props)
ax3.text(datetime(2019,1,30,11,10,0), 113, "Finish", color='r', fontsize=10, verticalalignment='top', horizontalalignment='center', bbox=props)

# Format x-axis
plt.xlim(datetime(2019,1,30,0,0,0),datetime(2019,1,30,23,59,59))
...
Line_Med = ax.errorbar(Median_Event21, MAX_Event21.index,   marker='o', c='blue',  markersize = 3.0, ls='-', xerr=MAD_Event21, capsize=2)
Line_Ran = ax.fill_betweenx(MAX_Event21.index, MAX_Event21, MIN_Event21, facecolor='blue', alpha=0.3, interpolate=False) # fill the distribution
Star_Max = ax.scatter(8.2, 0.3, c ='black', marker='*')
...
ax.scatter(Traj_E21['Traj Lon'], Traj_E21['Traj Lat'], zorder=2, cmap=cmap, transform=data_crs, marker='o', s=1, norm=norm, c=Traj_E21['IceContact_100m'], label='Traj Height (m)')
...
cs1 = ax.scatter(Traj_E21['Traj Age'], Traj_E21['Traj Height (m)'], marker='o', s=1, cmap=cmap, norm=norm, c=Traj_E21['IceContact_100m'], label='Traj Height (m)')
ax.plot(Traj_E21['Traj Age'][0], Traj_E21['Traj Height (m)'][0], color='k', marker='*')
