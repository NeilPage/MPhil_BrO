...
Data1 = pd.merge(left=SIPEXIIT,right=SIPEXII_Met, how='left', left_index=True, right_index=True)
Data2 = pd.merge(left=Data1,right=SIPEXII_SI, how='left', left_index=True, right_index=True)
Data3 = pd.merge(left=Data2,right=SIPEXII_Hg, how='left', left_index=True, right_index=True)
Data4 = pd.merge(left=Data3,right=SIPEXII_O3, how='left', left_index=True, right_index=True)
...
# Pass 2 (Temp) 
finiteY2mask = np.isfinite(Temp) # Scan for NaN values
BrO_Temp = BrO[finiteY2mask] # Remove NaN values from BrO
Temp = Temp[finiteY2mask] # Remove NaN values from Temp
Sol_Temp = Sol[finiteY2mask] # Remove NaN values from Sol

# Pass 3 (WD_vect) 
finiteY3mask = np.isfinite(WD_vect) # Scan for NaN values
BrO_WD = BrO[finiteY3mask] # Remove NaN values from BrO
WD_vect = WD_vect[finiteY3mask] # Remove NaN values from WD_vect
Sol_WD = Sol[finiteY3mask] # Remove NaN values from Sol

# Pass 4 (WS) 
finiteY4mask = np.isfinite(WS) # Scan for NaN values
BrO_WS = BrO[finiteY4mask] # Remove NaN values from BrO
WS = WS[finiteY4mask] # Remove NaN values from WS
Sol_WS = Sol[finiteY4mask] # Remove NaN values from Sol

# Pass 5 (Hg0) 
finiteY5mask = np.isfinite(Hg0) # Scan for NaN values
BrO_Hg0 = BrO[finiteY5mask] # Remove NaN values from BrO
Hg0 = Hg0[finiteY5mask] # Remove NaN values from SI
Sol_Hg0 = Sol[finiteY5mask] # Remove NaN values from Sol

# Pass 6 (SI Field) 
finiteY6mask = np.isfinite(SI) # Scan for NaN values
BrO_SI = BrO[finiteY6mask] # Remove NaN values from BrO
SI_SI = SI[finiteY6mask] # Remove NaN values from SI
Sol_SI = Sol[finiteY6mask] # Remove NaN values from Sol

#------------------------------------------------------------------------------
# Calculate the Coefficient of Correlation (r)

# 1) Between O3 and BrO
r_rowD1, p_valueD1 = stats.pearsonr(O3,BrO)
slopeD1, interceptD1, rD1, pD1, std_errD1= stats.linregress(O3,BrO)
...
plt.scatter(O3, BrO, edgecolors='none', marker='o', norm=norm, c=Sol, cmap=cmap)
...
plt.annotate("O$_3$ and BrO:\n (slope: "+str("%7.4f"%(slopeD1))+" $\pm$"+str("%7.4f"%(std_errD1))+" pptv, r: "+str("%7.4f"%(rD1))+")", xy=(7,12.5), color='black', fontweight='bold')
...
plt.annotate("Temperature and BrO:\n (slope: "+str("%7.4f"%(slopeD2))+" $\pm$"+str("%7.4f"%(std_errD2))+" $^\circ$C, r: "+str("%7.4f"%(rD2))+")", xy=(-14.0,12.5), color='black', fontweight='bold')
...
plt.annotate("Wind vector direction and BrO:\n (slope: "+str("%7.4f"%(slopeD3))+" $\pm$"+str("%7.4f"%(std_errD3))+" $^\circ$, r: "+str("%7.4f"%(rD3))+")", xy=(140,9.0), color='black', fontweight='bold')
...
plt.annotate("Wind speed and BrO:\n (slope: "+str("%7.4f"%(slopeD4))+" $\pm$"+str("%7.4f"%(std_errD4))+" m/s, r: "+str("%7.4f"%(rD4))+")", xy=(6,12.5), color='black', fontweight='bold')
...
plt.annotate("Solar radiation and BrO:\n (slope: "+str("%7.4f"%(slopeD5))+" $\pm$"+str("%7.4f"%(std_errD5))+" W/m$^2$, r: "+str("%7.4f"%(rD5))+")", xy=(260.0,9.0), color='black', fontweight='bold')
...
plt.annotate("Sea Ice Concentration and BrO:\n (slope: "+str("%7.4f"%(slopeD7))+" $\pm$"+str("%7.4f"%(std_errD7))+" %, r: "+str("%7.4f"%(rD7))+")", xy=(0.2,12.0), color='black', fontweight='bold')
