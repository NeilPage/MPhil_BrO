...
# BrO Data
V2_BrO = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/TG_Retrieval_V2/V2_BrO_QAQC.csv')

# NO2 Data
V2_NO2 = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/TG_Retrieval_V2/V2_NO2_QAQC.csv')

# O3 Data
V2_O3 = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/O3/V2_O3_1min.csv')

# Met data
V2_Met = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/V02/CAMMPCAN_V2_underway_60.csv') 

# Hg Data
V2_Hg = pd.read_csv('/Users/ncp532/Documents/Data/CAMMPCAN_2018_19/CAMMPCAN_V2_Hg0_Lat_Long_18-19.csv')

#------------------------------------------------------------------------------
# RESAMPLE DATASETS TOO 1 MIN RESOLUTION

# BrO Data  
V2_BrO['DateTime'] = pd.to_datetime(V2_BrO['DateTime'], format='%d/%m/%Y %H:%M:%S')
V2_BrO = V2_BrO.set_index('DateTime')
V2_BrO = V2_BrO.resample('1T').bfill()

# NO2 Data  
V2_NO2['DateTime'] = pd.to_datetime(V2_NO2['DateTime'], format='%d/%m/%Y %H:%M:%S')
V2_NO2 = V2_NO2.set_index('DateTime')
V2_NO2 = V2_NO2.resample('1T').bfill()

# O3 Data  
V2_O3['DateTime'] = pd.to_datetime(V2_O3['DateTime'], format='%d/%m/%Y %H:%M:%S')
V2_O3 = V2_O3.set_index('DateTime')

# Met Data  
V2_Met['DateTime'] = pd.to_datetime(V2_Met['DateTime'], format='%d/%m/%Y %H:%M:%S')
V2_Met = V2_Met.set_index('DateTime')

#------------------------------------------------------------------------------
# Add the Vector Mean Wind Direction to the Met data

# Latitude / Longitude
Lat_V1_18 = np.array(V2_Met['LATITUDE'])
Long_V1_18 = np.array(V2_Met['LONGITUDE'])

# Wind Direction
WD_s1_18 = np.array(V2_Met['WND_DIR_STRBD_CORR_DEG']) # starboard side wind direction (correlated)
WD_p1_18 = np.array(V2_Met['WND_DIR_PORT_CORR_DEG']) # port side wind direction (correlated)

# Wind Speed
WS_s1_18 = np.array(V2_Met['WND_SPD_STRBD_CORR_KNOT']) # starboard side wind speed (correlated)
WS_s1_18 = WS_s1_18 * 0.514444444 # Convert from knots to m/s
WS_p1_18 = np.array(V2_Met['WND_SPD_PORT_CORR_KNOT']) # port side wind speed (correlated)
WS_p1_18 = WS_p1_18 * 0.514444444 # Convert from knots to m/s
V2_Met['WS_1_18'] = ((WS_s1_18 + WS_p1_18)/2) # Average the wind speed for port and starboard
WS_1_18 = np.array(V2_Met['WS_1_18'])

V2_Met['WD_vect'] = ((WD_s1_18 * WS_s1_18) / (WS_s1_18 + WS_p1_18)) + ((WD_p1_18 * WS_p1_18) / (WS_s1_18 + WS_p1_18))
WD_vect1_18 = np.array(V2_Met['WD_vect'])
...
D3 = pd.merge(left=V2_BrO,right=V2_Met, how='left', left_index=True, right_index=True)
D4 = pd.merge(left=V2_NO2,right=V2_Met, how='left', left_index=True, right_index=True)
D5 = pd.merge(left=V2_O3,right=V2_Met, how='left', left_index=True, right_index=True)
...
O3_90 = D5['WD_vect'] <=60 # <=90
D10 = D5[O3_90]
O3_270 = D5['WD_vect'] >=190 # >=270 
D11 = D5[O3_270]
...
BrO_2 = np.array(V2_BrO['surf_vmr(ppmv)']) * 1e6

# NO2 surface volume mixing ratio (VMR)
NO2_vmr_V1_18 = np.array(DNO2['surf_vmr(ppmv)']) * 1e3 # convert from ppmv to ppbv
NO2_2 = np.array(V2_NO2['surf_vmr(ppmv)']) * 1e3

# Solar Radiation (W/m2)
Sol_s1_18 = np.array(V2_Met['RAD_SLR_STRBRD_WPERM2']) # starboard side solar radiation
Sol_p1_18 = np.array(V2_Met['RAD_SLR_PORT_WPERM2']) # port side solar radiation
Sol_V1_18 = np.array(V2_Met['Rad_Average'])

# Temperature (C)
Temp_s1_18 = np.array(V2_Met['TEMP_AIR_STRBRD_DEGC']) # starboard side temperature
Temp_p1_18 = np.array(V2_Met['TEMP_AIR_PORT_DEGC']) # port side temperature
Temp_V1_18 = (Temp_s1_18 + Temp_p1_18)/2 # Average the temperature for port and starboard

# Relative Humidity (RH)
RH_s1_18 = np.array(V2_Met['REL_HUMIDITY_STRBRD_PERCENT']) # starboard side relative humidity (%)
RH_p1_18 = np.array(V2_Met['REL_HUMIDITY_PORT_PERCENT']) # port side relative humidity (%)
RH_V1_18 = (RH_s1_18 + RH_p1_18)/2 # Average the RH for port and starboard (%)

# Atmospheric pressure (hPa)
Pres_s1_18 = np.array(V2_Met['ATM_PRESS_HPA']) # Atmospheric Pressure (hPa)

# O3 (ppb)
O3_V1_18 = np.array(DO3['O3'])*10 # all O3 data

# Hg Data
Hg0_V1_18 = np.array(V2_Hg['ng/m3']) # Hg0
RM_V1_18 = np.array(V2_Hg['RM_pg/m3']) # RM
RM_StDev = np.array(V2_Hg['RM_StDev']) # RM StDev
...
# V1_Met
dat3 = np.array(V2_Met['Date'])
tim3 = np.array(V2_Met['Time'])
dattim3 = dat3+' '+tim3
...
# V1_Hg0
dat5 = np.array(V2_Hg['Date'])
tim5 = np.array(V2_Hg['Time'])
dattim5 = dat5+' '+tim5
...
# BrO_2
dat6 = np.array(V2_BrO['Date'])
tim6 = np.array(V2_BrO['Time'])
dattim6 = dat6+' '+tim6
...
# NO2_2
dat7 = np.array(V2_NO2['Date'])
tim7 = np.array(V2_NO2['Time'])
dattim7 = dat7+' '+tim7
...
# O3_2
dat8 = np.array(V2_O3['Date'])
tim8 = np.array(V2_O3['Time'])
dattim8 = dat8+' '+tim8
...
# Format x-axis
#plt.xlim(datetime(2018,11,7),datetime(2018,11,17)) # at Davis station
...
plt.title('Time series for V2 (CAMMPCAN 2018-19)', fontsize=25, y=1.2)
...
#------------------------------
# Graph 4
ax=plt.subplot(814) # options graph 1 (vertical no, horizontal no, graph no)
